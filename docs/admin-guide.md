...
- Node.js 18+ и npm — для сборки/разработки фронтенда (не контейнеризован в `docker-compose.yml`).
...
| `minio` | 9000 (API), 9001 (консоль) | Хранилище фото обращений |
...
Все три уже вызываются сервисом `api-init` в `docker-compose.yml` при первом запуске.
...
- CORS сейчас открыт (`cors_origins: ["*"]` в `config.py`) — сузить под реальный домен фронтенда
  перед продакшеном.
